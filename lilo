  ...
  if [[ $OPTION == y ]]; then
    ...
    if [[ $OPTION == y ]]; then
      ...
      if [[ -f /home/${username}/.zshrc ]]; then
        ...
        if [[ $OPTION == y ]]; then
          sudo -u ${username} mv ~/.zshrc ~/.zshrc.bkp
          sudo -u ${username} cp /usr/share/oh-my-zsh/zshrc ~/.zshrc
        fi
      else
        sudo -u ${username} cp /usr/share/oh-my-zsh/zshrc ~/.zshrc
        sudo -u ${username} $EDITOR ~/.zshrc
      fi
    fi
    pause_function
  fi
...
install_desktop_environment(){
  install_icon_theme() { #{{{
    ...
    do
      ...
      echo " 1) $(menu_item "moka-icon-theme-git")"
  ...
  install_gtk_theme() { #{{{
    ...
    do
      ...
      echo " 1) $(menu_item "numix-themes" "Numix")"
      echo " 2) $(menu_item "zuki-themes-git" "Zukitwo")"
      echo ""
      echo " b) BACK"
      echo ""
      GTK_THEMES+=" b"
  ...
  install_themes() { #{{{
    ...
    do
      ...
      echo " 2) $(menu_item "numix-themes zuki-themes-git" "GTK Themes") $AUR"
      echo ""
      echo " d) DONE"
      echo ""
      THEMES_OPTIONS+=" d"
  ...
  echo " 2) Deepin          10) Fluxbox"
  echo " 3) Enlightenment   11) i3"
  echo " 4) GNOME           12) OpenBox"
  echo " 5) KDE             13) Xmonad"
  echo " 6) LXQT"
  echo " 7) Mate"
  echo " 8) XFCE"
  echo ""
  ...
  case "$OPTION" in
    ...
    5)
      ...
      package_install "plasma kde-applications kde-l10n-$LOCALE_KDE sddm"
      # config xinitrc
...
install_accessories_apps(){
  ...
  do
    ...
    echo " 3) $(menu_item "compiz") $AUR"
    echo " 4) $(menu_item "conky-lua") $AUR"
    echo " 5) $(menu_item "docky") $AUR"
    echo " 6) $(menu_item "galculator") $AUR"
    ...
    echo "14) $(menu_item "unified-remote-server" "Unified Remote")"
    echo "15) $(menu_item "zim")"
    echo "16) $(menu_item "plank")"
    echo ""
    echo " b) BACK"
    echo ""
  ...
  do
    ...
    for OPT in ${OPTIONS[@]}; do
      case "$OPT" in
        ...
        10)
          package_remove "jdk"
          package_install "jdk7-openjdk icedtea-web"
          ;;
        11)
      ...
      case "$OPT" in
        ...
        8)
          ...
          if [[ -z ${VIRTUALBOX_GUEST} ]]; then
            package_install "virtualbox virtualbox-host-modules virtualbox-guest-iso qt4"
      ...
      case "$OPT" in
        ...
        2)
          ...
          do
            ...
            echo " 3) $(menu_item "google-musicmanager" "Google Music Manager")"
            echo " 4) $(menu_item "ocenaudio") $AUR"
            echo " 5) $(menu_item "soundconverter soundkonverter" "$([[ ${KDE} -eq 1 ]] && echo "Soundkonverter" || echo "Soundconverter";)")"
            echo ""
            echo " b) BACK"
            echo ""
            ...
            for OPT in ${OPTIONS[@]}; do
              case "$OPT" in
                ...
                2)
                  ...
                  ;;
                3)
                  aur_package_install "google-musicmanager"
                  ;;
                4)
      ...
      case "$OPT" in
        1)
          ...
          do
            ...
            echo " 7) $(menu_item "popcorntime-bin") $AUR"
            echo " 8) $(menu_item "smplayer")"
