...
install_desktop_environment(){
  install_icon_theme() { #{{{
    ...
    do
      ...
      echo " 3) $(menu_item "moka-icon-theme-git")"
  ...
  echo " --- DE ---             --- WM ---"
  echo " 1) Cinnamon            7) Awesome"
  echo " 2) E17                 8) Fluxbox"
  echo " 3) GNOME               9) OpenBox"
  echo " 4) KDE"
  echo " 5) LXDE"
  echo " 6) XFCE"
  echo ""
...
install_web_server(){
  ...
  install_postgis(){ #{{{
    ...
    sudo -u ${USER_NAME} psql -d postgres -c "UPDATE pg_database SET datistemplate=FALSE WHERE datname='template1';"
    sudo -u ${USER_NAME} psql -d postgres -c "DROP DATABASE template1;"
    sudo -u ${USER_NAME} psql -d postgres -c "CREATE DATABASE template1 WITH TEMPLATE = template0 ENCODING = 'UNICODE';"
    sudo -u ${USER_NAME} psql -d postgres -c "UPDATE pg_database SET datistemplate = TRUE WHERE datname = 'template1';"
    sudo -u ${USER_NAME} psql -d postgres -c "\c template1"
    sudo -u ${USER_NAME} psql -d postgres -c "VACUUM FREEZE;"
    sudo -u ${USER_NAME} psql -d postgres -c "UPDATE pg_database SET datallowconn=FALSE WHERE datname='template1';"
    ...
    sudo -u ${USER_NAME} psql -d postgres -c "UPDATE pg_database SET datistemplate=FALSE WHERE datname='template_postgis';"
    sudo -u ${USER_NAME} dropdb template_postgis

    sudo -u ${USER_NAME} createdb -E UTF8 template_postgis
    sudo -u ${USER_NAME} createlang -d template_postgis plpgsql
    sudo -u ${USER_NAME} psql -d postgres -c "UPDATE pg_database SET datistemplate='true' WHERE datname='template_postgis';"

    #PostGIS:
        sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/postgis.sql
        sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/spatial_ref_sys.sql
        #psql -d template_postgis -f $POSTGIS_PATH/postgis_comments.sql
    #with raster support:
        sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/rtpostgis.sql
        #sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/raster_comments.sql
    #with topology support:
        #sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/topology.sql
        #sudo -u ${USER_NAME} psql -d template_postgis -f $POSTGIS_PATH/topology_comments.sql

    sudo -u ${USER_NAME} psql -d template_postgis -c "GRANT ALL ON geometry_columns TO PUBLIC;"
    sudo -u ${USER_NAME} psql -d template_postgis -c "GRANT ALL ON geography_columns TO PUBLIC;"
    sudo -u ${USER_NAME} psql -d template_postgis -c "GRANT ALL ON spatial_ref_sys TO PUBLIC;"
