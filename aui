...
#COMMON FUNCTIONS {{{
  ...
  check_vga() { #{{{
    ...
    ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Detecting video chipset "
  ...
  update_early_modules() { #{{{
    ...
    if [[ -n ${NEW_MODULE} ]]; then
      ...
      if [ $? -eq 1 ]; then
        ...
        ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Rebuilding init "
  ...
  aur_package_install(){ #{{{
    ...
    for PKG in $1; do
      if ! is_package_installed "${PKG}" ; then
        if [[ $AUTOMATIC_MODE -eq 1 ]]; then
          ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Installing ${AUR} ${BWhite}${PKG}${White} "
        ...
        if [[ $VERBOSE_MODE -eq 0 ]]; then
          cecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Installing ${AUR} ${BWhite}${PKG}${White} success"
        else
  ...
  package_install(){ #{{{
    ...
    if [[ $AUTOMATIC_MODE -eq 1 || $VERBOSE_MODE -eq 0 ]]; then
      for PKG in ${1}; do
        ...
        if ! is_package_installed "${PKG}" ; then
          ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Installing (${PKG_REPO}) ${BWhite}${PKG}${White} "
          pacman -S --noconfirm --needed ${PKG} >>"$LOG" 2>&1 &
          pid=$!;progress $pid
        else
          cecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Installing (${PKG_REPO}) ${BWhite}${PKG}${White} exists "
        fi
      done
    else
      pacman -S --needed $1
  ...
  package_remove(){ #{{{
    ...
    for PKG in ${1}; do
      if is_package_installed "${PKG}" ; then
        if [[ $AUTOMATIC_MODE -eq 1 || $VERBOSE_MODE -eq 0 ]]; then
          ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Removing ${BWhite}${PKG}${White} "
  ...
  system_upgrade(){ #{{{
    if [[ $VERBOSE_MODE -eq 0 ]]; then
      ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Upgrading packages "
  ...
  system_update(){ #{{{
    if [[ $VERBOSE_MODE -eq 0 ]]; then
      ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Updating packages "
      pacman -Sy >>"$LOG" 2>&1 &
      pid=$!;progress $pid
    else
      pacman -Sy $1
  ...
  add_user_to_group() { #{{{
    ...
    ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Adding ${BWhite}${_USER}${White} to ${BWhite}${_GROUP}${White} "
  ...
  system_ctl () { #{{{
    local ACTION=${1}
    local OBJECT=${2}
    ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} systemctl ${ACTION} ${OBJECT} "
...
case "$1" in
  ...
  *)
  ...
  language_selector(){
    ...
    #}}}
    #FIREFOX #{{{
    if [[ $LOCALE == pt_BR || $LOCALE == pt_PT || $LOCALE == en_GB || $LOCALE == es_AR || $LOCALE == es_ES || $LOCALE == zh_CN ]]; then
  ...
  install_basic_setup(){
    ...
    package_install "ntp"
    is_package_installed "ntp" && system_ctl enable ntpd
    pause_function
    ...
    is_package_installed "avahi" && system_ctl enable avahi-dnsconfd.service
    pause_function
    ...
    is_package_installed "acpid" && system_ctl enable acpid.service
    pause_function
  ...
  install_ssh(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable sshd.service
  ...
  install_nfs(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable rpc-statd.service
      pause_function
    fi
  }
  #}}}
  ...
  install_samba(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable smbd.service
      system_ctl enable nmbd.service
      system_ctl enable winbindd.service
      pause_function
    fi
  }
  #}}}
  ...
  install_preload(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable preload.service
      pause_function
    fi
  }
  #}}}
  ...
  install_zram (){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable zramswap.service
      pause_function
    fi
  }
  #}}}
  ...
  install_tlp(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      tlp start
      system_ctl enable tlp-init.service
      pause_function
    fi
  }
  #}}}
  ...
  install_fwd(){
    ...
    read_input_text "Install FirewallD" $FWD
    if [[ $OPTION == y ]]; then
  ...
  install_video_cards(){
    ...
    if [[ ${VIDEO_DRIVER} == virtualbox ]]; then
      ...
      system_ctl enable vboxservice.service
      VBoxClient-all
    #}}}
    #NVIDIA {{{
    ...
    #Catalyst [ATI] {{{
      ...
      [[ -f /etc/modules-load.d/radeon.conf ]] && rm /etc/modules-load.d/radeon.conf
    ...
    #ATI {{{
      ...
      pacman -S --asdeps mesa-libgl
      [[ -f /etc/modules-load.d/ati.conf ]] && rm /etc/modules-load.d/ati.conf
      [[ -f /etc/modules-load.d/fglrx.conf ]] && rm /etc/modules-load.d/ati.conf
      package_install "xf86-video-${VIDEO_DRIVER} ${VIDEO_DRIVER}-dri"
      if [[ ${ARCHI} == x86_64 ]]; then
  ...
  install_git_tor(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl start tor.service
      system_ctl start privoxy.service
      system_ctl enable tor.service
      system_ctl enable privoxy.service
      pause_function
    fi
  }
  #}}}
  ...
  install_desktop_environment(){
    ...
    install_gtk_theme() { #{{{
      ...
      do
        ...
        echo " 5) $(menu_item "faience-themes" "Faience")"
        echo " 6) $(menu_item "gtk-theme-gnome-cupertino" "Gnome Cupertino")"
        echo " 7) $(menu_item "mediterraneannight-theme" "MediterraneanNight")"
        echo " 8) $(menu_item "orion-gtk-theme" "Orion")"
        echo " 9) $(menu_item "ubuntu-themes") (aka Ambiance/Radiance)"
        ...
        for OPT in ${OPTIONS[@]}; do
          case "$OPT" in
            ...
            8)
              ...
              ;;
            9)
              aur_package_install "ubuntu-themes"
              ;;
            10)
        ...
        for OPT in ${OPTIONS[@]}; do
          case "$OPT" in
            1)
              ...
              system_ctl enable entrace.service
              ;;
            2)
              package_install "gdm"
            ...
            3)
              ...
              system_ctl enable lightdm.service
              ;;
            4)
              package_install "lxdm"
              system_ctl enable lxdm.service
              ;;
            5)
              package_install "slim"
              system_ctl enable slim.service
              ;;
            "b")
              break
              ;;
    ...
    install_themes() { #{{{
      ...
      do
        ...
        [[ $GNOME -eq 1 ]] && echo " 3) $(menu_item "gnome-shell-theme-elegance gnome-shell-theme-eos" "Gnome Shell Themes") $AUR"
    ...
    case "$OPTION" in
      1)
        ...
        package_install "cinnamon-screensaver cinnamon-control-panel"
  ...
  install_accessories_apps(){
    ...
    do
      ...
      echo " 8) $(menu_item "terra-bzr") $AUR"
  ...
  install_development_apps(){
    ...
    do
      ...
      echo "10) $(menu_item "jdk7-openjdk" "OpenJDK") $AUR"
      echo "11) $(menu_item "jdk" "Oracle JDK") $AUR"
      echo "12) $(menu_item "qtcreator")"
      echo "13) $(menu_item "sublime-text" "Sublime Text 2") $AUR"
      echo "14) $(menu_item "gdb" "Debuggers & Syntax Tools")"
      echo "15) $(menu_item "mysql-workbench-gpl" "MySQL Workbench") $AUR"
      echo "16) $(menu_item "meld")"
      echo "17) $(menu_item "rabbitvcs" "RabbitVCS") $AUR"
      echo ""
      echo " b) BACK"
      echo ""
      ...
      for OPT in ${OPTIONS[@]}; do
        case "$OPT" in
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  ...
                  2)
                    ...
                    ;;
                  3)
                    aur_package_install "android-sdk android-apktool android-sdk-platform-tools android-udev eclipse-android simple-mtpfs"
                    groupadd android
          ...
          14)
            ...
            aur_package_install "closure-compiler nodejs-jslint" #Javascript
            aur_package_install "tidyhtml" #HTML
            aur_package_install "flake8" #Python
            # syntax tools
        ...
        case "$OPT" in
          ...
          4)
            ...
            if [[ -z ${VIRTUALBOX_GUEST} ]]; then
              ...
              add_module "vboxdrv" "virtualbox-host"
              modprobe -a vboxdrv
              add_user_to_group ${USER_NAME} vboxusers
            else
              cecho " ${BBlue}[${White}${BWhite}!${White}${BBlue}]${White} VirtualBox was not installed as we are a VirtualBox guest."
            fi
            ;;
          5)
            package_install "webmin perl-net-ssleay"
            system_ctl enable webmin.service
            ;;
          6)
            package_install "icoutils wine wine_gecko winetricks"
            aur_package_install "wine-mono-bin"
            ;;
          "b")
            break
            ;;
    ...
    do
      ...
      for OPT in ${OPTIONS[@]}; do
        case "$OPT" in
          1)
            #BROWSER {{{
            is_package_installed "epiphany" && package_remove "epiphany"
            while [[ 1 ]]
            do
              print_title "BROWSER"
          ...
          2)
            ...
            do
              ...
              echo " 7) $(menu_item "spideroak") $AUR"
              echo " 8) $(menu_item "steadyflow-bzr") $AUR"
              echo " 9) $(menu_item "transmission-qt transmission-gtk" "Transmission")"
              echo "10) $(menu_item "ubuntuone-client" "Ubuntu One") $AUR"
              echo ""
              echo " b) BACK"
              echo ""
              DOWNLOAD_OPTIONS+=" b"
          ...
          3)
            ...
            do
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  1)
                    package_install "thunderbird thunderbird-i18n-$LOCALE_FF"
                    ;;
                  2)
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  ...
                  8)
                    ...
                    ;;
                  9)
                    package_install "rhythmbox libgpod libdmapsharing"
                    aur_package_install "rhythmbox-equalizer-git"
                    ;;
                  10)
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  ...
                  2)
                    ...
                    add_user_to_group ${USER_NAME} "lol"
                    ;;
                  3)
  ...
  install_web_server(){
    create_sites_folder(){
      ...
      replaceinfile 'public_html' 'Sites' /etc/httpd/conf/extra/httpd-userdir.conf
    ...
    echo " 1) LAMP - APACHE, MariaDB & PHP + ADMINER"
    ...
    case "$OPTION" in
      1)
        package_install "apache mariadb php php-apache php-mcrypt php-gd"
        ...
        if [[ ! -f  /etc/httpd/conf/httpd.conf.aui ]]; then
          ...
          replaceinfile 'DirectoryIndex\ index.html' 'DirectoryIndex\ index.html\ index.php' /etc/httpd/conf/httpd.conf
        fi
        #}}}
        #CONFIGURE PHP.INI {{{
  ...
  install_fonts(){
    ...
    do
      ...
      echo " 2) $(menu_item "ttf-dejavu")"
      echo " 3) $(menu_item "ttf-exljbris") $AUR"
      echo " 4) $(menu_item "ttf-funfonts") $AUR"
      echo " 5) $(menu_item "ttf-google-webfonts") $AUR"
      echo " 6) $(menu_item "ttf-liberation")"
        ...
        case "$OPT" in
          1)
            ...
            do
              ...
              echo " 2) $(menu_item "cerbere-bzr") session manager"
              echo " 3) $(menu_item "contractor-bzr") (sharing service)"
              echo " 4) $(menu_item "eidete-bzr") (screencasting)"
              echo " 5) $(menu_item "dexter-contacts-bzr") (contacts manager)"
              echo " 6) $(menu_item "feedler-bzr") (RSS feeds Reader)"
              echo " 7) $(menu_item "pantheon-files-bzr") (file manager)"
              echo " 8) $(menu_item "footnote-bzr") (note taking)"
              echo " 9) $(menu_item "foto-bzr") (photo manager)"
              echo "10) $(menu_item "gala-bzr") (compositing manager)"
              echo "11) $(menu_item "geary-git") (email client)"
              echo "12) $(menu_item "lingo-dictionary-bzr") (dictionary)"
              echo "13) $(menu_item "maya-bzr") (calendar)"
              echo "14) $(menu_item "midori") (web browser)"
              echo "15) $(menu_item "noise-bzr") (audio player)"
              echo "16) $(menu_item "scratch-bzr") (text editor)"
              echo "17) $(menu_item "pantheon-dock-bzr") (dock)"
              echo "18) $(menu_item "pantheon-terminal-bzr") (terminal)"
              echo "19) $(menu_item "slingshot-bzr") (app launcher)"
              echo "20) $(menu_item "snap-elementary-bzr") (photo booth)"
              echo "21) $(menu_item "switchboard-bzr") (desktop settings hub)"
              echo "22) $(menu_item "wingpanel-bzr") (indicators topbar)"
              echo "23) $(menu_item "elementary-icons-bzr" "elementary icons")"
              echo "24) $(menu_item "egtk-bzr" "elementary theme")"
              echo ""
              echo " b) BACK"
              echo ""
