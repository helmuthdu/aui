...
install_desktop_environment(){
  ...
  case "$OPTION" in
    1)
      ...
      system_ctl enable upower
      #Improvements
      local MAX_WTCHES=`cat /etc/sysctl.conf | grep fs.inotify.max_user_watches`
      [[ -z $MAX_WTCHES ]] && echo "fs.inotify.max_user_watches = 524288" >> /etc/sysctl.conf
      ;;
      #}}}
      4)
      #KDE {{{
...
install_development_apps(){
  ...
  do
    ...
    echo "15) $(menu_item "sublime-text" "Sublime Text") $AUR"
    echo "16) $(menu_item "gdb" "Debuggers")"
    echo "17) $(menu_item "mysql-workbench-gpl" "MySQL Workbench") $AUR"
    echo "18) $(menu_item "sql-power-architect" "SQL Power Architect") $AUR"
    echo "19) $(menu_item "meld")"
    echo "20) $(menu_item "rabbitvcs" "RabbitVCS") $AUR"
    echo ""
    echo " b) BACK"
    echo ""
            ...
            for OPT in ${OPTIONS[@]}; do
              case "$OPT" in
                ...
                2)
                  ...
                  ;;
                3)
                  aur_package_install "android-sdk android-apktool android-sdk-platform-tools android-udev eclipse-android libmtp"
                  if [[ ${KDE} -eq 1 ]]; then
                    package_install "kio-mtp"
                  else
                    package_install "gvfs-mtp"
...
install_web_server(){
  ...
  echo " 3) LEMP - NGINX, MariaDB & PHP + ADMINER"
  echo " 4) LEPP - NGINX, POSTGRESQL & PHP + ADMINER"
  echo ""
  echo " b) BACK"
  echo ""
