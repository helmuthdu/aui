  ...
  do
    ...
    case "$OPTION" in
      1)
        ...
        is_package_installed "fontconfig-infinality-ultimate" && pacman -Rdds --noconfirm freetype2-infinality-ultimate fontconfig-infinality-ultimate cairo-infinality-ultimate
      ...
      2)
        ...
        add_repository "infinality-bundle" "http://ibn.net63.net/infinality-bundle/\$arch"
        [[ $ARCHI == x86_64 ]] && add_repository "infinality-bundle-multilib" "http://ibn.net63.net/infinality-bundle-multilib/\$arch"
        is_package_installed "fontconfig" && pacman -Rdds --noconfirm freetype2 fontconfig cairo
        is_package_installed "fontconfig-ubuntu" && pacman -Rdds freetype2-ubuntu fontconfig-ubuntu cairo-ubuntu
...
install_desktop_environment(){
  ...
  case "$OPTION" in
    1)
      ...
      add_repository "mate" "http://repo.mate-desktop.org/archlinux/\$arch" "Optional TrustAll"
...
install_web_server(){
  ...
  echo " 3) LAMA - APACHE, MariaDB & ASP.NET"
  echo " 4) LAPA - APACHE, POSTGRESQL & ASP.NET"
  echo " 5) LEMP - NGINX, MariaDB & PHP"
  echo " 6) LEPP - NGINX, POSTGRESQL & PHP"
  echo ""
  echo " b) BACK"
  echo ""
  ...
  case "$OPTION" in
    1)
      ...
      #ADD PHP SUPPORT {{{
        ...
        [[ -f /etc/httpd/conf/httpd.conf.aui ]] && echo "/etc/php/php.ini.aui" || cp -v /etc/httpd/conf/httpd.conf /etc/httpd/conf/httpd.conf.aui
    ...
    2)
      ...
      ;;
    3)
      package_install "apache mariadb mono mod_mono xsp"
      install_adminer
      system_ctl enable httpd
      /usr/bin/mysql_install_db --user=mysql --basedir=/usr --datadir=/var/lib/mysql
      system_ctl enable mysqld
      ...
      /usr/bin/mysql_secure_installation
      #ADD ASP.NET SUPPORT {{{
        local MOD_MONO=`cat /etc/httpd/conf/httpd.conf | grep mod_mono`
        if [[ -z $MOD_MONO ]]; then
          echo -e '\n# ASP.NET Configuration\nInclude /etc/httpd/conf/mod_mono.conf' >> /etc/httpd/conf/httpd.conf
        fi
      #}}}
      create_sites_folder
      ;;
    4)
      package_install "apache postgresql mono mod_mono xsp"
      install_adminer
      systemd-tmpfiles --create postgresql.conf
      mkdir -p /var/lib/postgres
      chown -R postgres:postgres /var/lib/postgres
    ...
    for OPT in ${OPTIONS[@]}; do
      case "$OPT" in
        ...
        2)
          ...
          if [[ ! -f /home/${USER_NAME}/.config/gtk-3.0/settings.ini ]]; then
            sudo -u ${USER_NAME} echo -e "[Settings]\ngtk-shell-shows-menubar = 1" > /home/${USER_NAME}/.config/gtk-3.0/settings.ini
          else
            add_line "gtk-shell-shows-menubar = 1" "/home/${USER_NAME}/.config/gtk-3.0/settings.ini"
          fi
          ;;
        "b")
          break
